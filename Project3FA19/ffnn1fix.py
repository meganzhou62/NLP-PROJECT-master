...
class FFNN(nn.Module):
	def __init__(self, input_dim, h):
			...
			self.h = h
			self.W1 = nn.Linear(input_dim, h)  #Applies a linear transformation to the incoming data https://pytorch.org/docs/stable/nn.html?highlight=linear#torch.nn.Linear
			self.activation = nn.ReLU() # The rectified linear unit; one valid choice of activation function
			self.W2 = nn.Linear(h, 5)
			### should we have h output
...
def convert_to_vector_representation(data, word2index):
	...
	for document, y in data:
		vector = torch.zeros(len(word2index))  #Returns a tensor filled with the scalar value 0, with the shape defined by the variable argument size
